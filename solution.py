"""
==============================================================
Day 10 Lab: Build Your First Automated ETL Pipeline
==============================================================
Student ID: HieuGM
Name: HieuGM

Nhiem vu:
   1. Extract:   Doc du lieu tu file JSON
   2. Validate:  Kiem tra & loai bo du lieu khong hop le
   3. Transform: Chuan hoa category + tinh gia giam 10%
   4. Load:      Luu ket qua ra file CSV

Cham diem tu dong:
   - Script phai chay KHONG LOI (20d)
   - Validation: loai record gia <= 0, category rong (10d)
   - Transform: discounted_price + category Title Case (10d)
   - Logging: in so record processed/dropped (10d)
   - Timestamp: them cot processed_at (10d)
==============================================================
"""

import json
import pandas as pd
import datetime

# --- CONFIGURATION ---
SOURCE_FILE = 'raw_data.json'
OUTPUT_FILE = 'processed_data.csv'
EXPECTED_COLUMNS = ['id', 'product', 'price', 'category']


def extract(file_path):
    """
    Task 1: Doc du lieu JSON tu file.

    Goi y:
       - Dung json.load() de doc file JSON
       - Xu ly truong hop file khong ton tai (FileNotFoundError)

    Returns:
        list: Danh sach cac records (dictionaries)
    """
    print(f"Extracting data from {file_path}...")
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
    except FileNotFoundError:
        print(f"Extract failed: {file_path} was not found.")
        return []
    except json.JSONDecodeError as exc:
        print(f"Extract failed: invalid JSON ({exc}).")
        return []

    if not isinstance(data, list):
        print("Extract failed: JSON root must be a list of records.")
        return []

    print(f"Extracted {len(data)} records from {file_path}.")
    return data


def validate(data):
    """
    Task 2: Kiem tra chat luong du lieu.

    Quy tac validation:
       - Price phai > 0 (loai bo gia am hoac bang 0)
       - Category khong duoc rong

    Goi y:
       - Dung record.get('price', 0) de lay gia
       - Dung record.get('category') de kiem tra category
       - In ra so luong record hop le va khong hop le

    Returns:
        list: Danh sach cac records hop le
    """
    valid_records = []
    error_count = 0

    for index, record in enumerate(data, start=1):
        if not isinstance(record, dict):
            error_count += 1
            print(f"Record {index} dropped: not a JSON object.")
            continue

        try:
            price = float(record.get('price', 0))
        except (TypeError, ValueError):
            error_count += 1
            print(f"Record {index} dropped: price is not numeric.")
            continue

        category = record.get('category')
        if price <= 0:
            error_count += 1
            print(f"Record {index} dropped: price must be greater than 0.")
            continue

        if category is None or str(category).strip() == '':
            error_count += 1
            print(f"Record {index} dropped: category is empty.")
            continue

        clean_record = record.copy()
        clean_record['price'] = price
        clean_record['category'] = str(category).strip()
        valid_records.append(clean_record)

    print(
        f"Validation summary: {len(valid_records)} valid records kept, "
        f"{error_count} invalid records dropped."
    )
    return valid_records


def transform(data):
    """
    Task 3: Ap dung business logic.

    Yeu cau:
       - Tinh discounted_price = price * 0.9 (giam 10%)
       - Chuan hoa category thanh Title Case (vi du: "electronics" -> "Electronics")
       - Them cot processed_at = timestamp hien tai

    Goi y:
       - Dung pd.DataFrame(data) de tao DataFrame
       - df['discounted_price'] = df['price'] * 0.9
       - df['category'] = df['category'].str.title()
       - df['processed_at'] = datetime.datetime.now().isoformat()

    Returns:
        pd.DataFrame: DataFrame da duoc transform
    """
    df = pd.DataFrame(data, columns=EXPECTED_COLUMNS if not data else None)

    if 'price' not in df.columns:
        df['price'] = pd.Series(dtype='float64')
    if 'category' not in df.columns:
        df['category'] = pd.Series(dtype='object')

    df['price'] = pd.to_numeric(df['price'], errors='coerce')
    df['category'] = df['category'].astype(str).str.strip().str.title()
    df['discounted_price'] = (df['price'] * 0.9).round(2)
    df['processed_at'] = datetime.datetime.now().isoformat(timespec='seconds')

    print(f"Transform complete. {len(df)} records processed.")
    return df


def load(df, output_path):
    """
    Task 4: Luu DataFrame ra file CSV.

    Goi y:
       - df.to_csv(output_path, index=False)
    """
    df.to_csv(output_path, index=False)
    print(f"Loaded {len(df)} records into {output_path}.")


# ============================================================
# MAIN PIPELINE
# ============================================================
if __name__ == "__main__":
    print("=" * 50)
    print("ETL Pipeline Started...")
    print("=" * 50)

    # 1. Extract
    raw_data = extract(SOURCE_FILE)

    # 2. Validate
    clean_data = validate(raw_data)

    # 3. Transform
    final_df = transform(clean_data)

    # 4. Load
    if final_df is not None:
        load(final_df, OUTPUT_FILE)
        print(f"\nPipeline completed! {len(final_df)} records saved.")
    else:
        print("\nTransform returned None. Check your transform() function.")
